...
font <- "Special Gothic" # "Special Gothic" would be close to Costa font but it doesn't work for some reason.
font_b <- "Open Sans"
showtext_opts(dpi = 300)
font_add_google(font, db_cache = FALSE)
...
p1 <- plot_by_decade %>%
  ...
  geom_col(width = 0.8, color = col_bg) +
  geom_text(
  ...
  labs(x = NULL, y = NULL,
       title = "Gender Distribution of Costa Book Awards",) +
  theme(
  ...
  annotate(geom = "text", x = 0, y = 8,
           label = "The Costa Book Awards (Whitbread Book Awards from 1971 to 2005) were a set of annual \nliterary awards from 1971 to 2021 recognising English-language books by writers based in \nUK and Ireland. Here is the gender distribution of Costa Book Award winners by decade \nfrom 1991 to 2022.",
           color = col_t, hjust = 0, vjust = 1, size = 2.5, family = font) +
  annotate(geom = "text", x = 27, y = 4.7,
           label = "All Costa Book Award winners (1991-2022)",
...
label_percent2 <- plot_by_decade_and_prize_alias %>% 
  mutate(percent = if_else(percent > 29, percent, NA),
         percent = round(percent, 1), 
         percent = as.character(percent), 
         percent = if_else(is.na(percent), NA, paste0(percent, '%'))) %>% 
  pull(percent)
