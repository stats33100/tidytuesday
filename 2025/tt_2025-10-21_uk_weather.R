...
wettest_avg <- max(year_month_rainfall$avg)
wettest_year <- filter(year_month_rainfall, avg == wettest_avg) %>% pull(year)
wettest_month <- filter(year_month_rainfall, avg == wettest_avg) %>% pull(month)
...
year_month_rainfall %>% 
  ...
  labs(
    ...
    subtitle = glue("The wettest months in UK are on average January, October, November and December. \nThe wettest single month in UK's history has been October 1903, with rainfall of {round(wettest_avg, 1)} mm \n(according to the data provided)."),
  ...
  annotate(
      geom = "text", 
      x = wettest_month, y = wettest_avg, 
      label = glue("{wettest_year}"),
      family = font,
      hjust = 1.3,
      size = 2.5) +
  annotate(
